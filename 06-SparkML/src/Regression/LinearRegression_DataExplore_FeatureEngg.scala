...
object LinearRegression_DataExplore_FeatureEngg {
   ...
   def main(args:Array[String]): Unit = {
    ...
    val df_OutletSizeNull =
      fill_MissingValues_ItemWeight.filter(fill_MissingValues_ItemWeight("Outlet_Size").isNull)
   ...
   // val modified_Df = new_Df.na.replace("Outlet_Size", Map("" -> "NA"))
    val modified_Df = new_Df.na.fill("NA", Seq("Outlet_Size"))
    modified_Df.show()
    ...
    val missing_Rows = replaced_MissingValues_ForOutletSize
      .filter (replaced_MissingValues_ForOutletSize("Outlet_Size").isNull)
    ...
    val new_Df_WithItem_Fat_Content_Combined = revised_Df
      .withColumn("Item_Fat_Content_Combined",sqlFunc3(col
      ("Item_Fat_Content")))
